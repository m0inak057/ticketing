...
def generate_invoice_pdf(invoice):
    ...
    try:
        ...
        logger.warning(f"Could not load NexGen FC logo: {e}")
    ...
    pricing_data = [
        ['Description', 'Amount (₹)'],
        ['Base Ticket Price', f"₹{invoice.base_price}"],
        ['TapNex Service Fee', f"₹{invoice.commission}"],
        ['', ''],
        ['Total Paid', f"₹{invoice.total_price}"],
    ]
